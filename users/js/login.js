...
ons.ready(function(){
var mesin = localStorage.getItem("mesin");
if(!mesin){
$("#lock-page").css("display","block");
$.get(serverweb+"mesin_id.php?token=wxy1",function(restconf){
localStorage.setItem("mesin",restconf);
window.location.href="./login.html";
});
}else{
var machine = JSON.parse(mesin);
...
var hash = window.location.hash;
  ...
  captcha();
  $("#appname").html(machine.appname);
  $("#versi").html(machine.versi);
  ...
  $.get(serverweb+"userproses.php?key=semangat&opt=masuk&email="+username+"&pass="+password,function(result){
  //alert(result);
  if(result == 0){
  wait.fadeOut();
...
var daftar = function() {
	...
	$.get(serverweb+"userproses.php?key=semangat&opt=daftar&nama="+d_nama+"&pass="+d_password+"&email="+d_email,function(result){
	if(result ==1){
	wait.html("Pendaftaran akun kamu berhasil,,Silahkan lanjutkan untuk login!");
	$("#d-nama").val("");
	$("#d-email").val("");
	$("#d-password").val("");
	$("#d-password2").val("");
	...
	/*
  $.get(serverweb+"userproses.php?key=semangat&opt=daftar&email="+username+"&pass="+password,function(result){
  //alert(result);
  if(result == 0){
  wait.fadeOut();
  $("#result").html("Cek kembali data akun yang kamu masukan!");
